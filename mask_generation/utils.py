...
Extracted from utils_stableviton.py
"""
import numpy as np
import cv2
from PIL import Image, ImageDraw

# Label mapping for human parsing
LABEL_MAP = {
    "background": 0,
    "hat": 1,
    "hair": 2,
...
def extend_arm_mask(wrist, elbow, scale):
    """Extend arm mask beyond wrist"""
    wrist = elbow + scale * (wrist - elbow)
    return wrist


def hole_fill(img):
    """Fill holes in mask using flood fill"""
    ...
    # Apply additional morphological operations to fill remaining holes
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
    dst = cv2.morphologyEx(dst, cv2.MORPH_CLOSE, kernel, iterations=2)
...
def refine_mask(mask):
    """Refine mask by keeping only the largest contour"""
...
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, 
                     width=384, height=512, radius=5, neck_mask_radius=0.3, mask_shoes=False):
    """
    Generate mask location based on parsing and keypoints
    
    Args:
        model_type: 'hd' or 'dc'
        category: 'upper_body', 'lower_body', or 'dresses'
        model_parse: Parsed image from human parsing model
        keypoint: Keypoint dictionary with 'pose_keypoints_2d'
        width: Image width
        height: Image height
        radius: Dilation radius
        neck_mask_radius: Neck mask radius (0.0=no neck mask, 1.0=full neck mask, 0.3=bottom 30% masked)
        mask_shoes: If True, include shoes in the mask (default: False, shoes are kept visible)
    
    Returns:
        Tuple of (mask, mask_gray)
    """
    ...
    # Build parser_mask_fixed - items that are NOT masked (kept visible)
    parser_mask_fixed = (parse_array == LABEL_MAP["hat"]).astype(np.float32) + \
                        (parse_array == LABEL_MAP["sunglasses"]).astype(np.float32) + \
                        (parse_array == LABEL_MAP["bag"]).astype(np.float32)
    
    # Conditionally add shoes to parser_mask_fixed (if mask_shoes=False, shoes are kept visible)
    if not mask_shoes:
    ...
    if category == 'dresses':
        ...
        # Include shoes in mask if mask_shoes=True (for baggy_lower and ethnic_combined)
        if mask_shoes:
            parse_mask += (parse_array == LABEL_MAP["left_shoe"]).astype(np.float32) + \
                         (parse_array == LABEL_MAP["right_shoe"]).astype(np.float32)
    ...
    if category == 'dresses' or category == 'upper_body':
        # Add configurable neck mask with radius control
        if neck_mask_radius > 0:
            neck_mask = (parse_array == 18).astype(np.float32)
            neck_mask = cv2.dilate(neck_mask, np.ones((radius, radius), np.uint16), iterations=1)
            neck_mask = np.logical_and(neck_mask, np.logical_not(parse_head))
            
            # Apply neck mask radius (0.0 = no neck mask, 1.0 = full neck mask)
            if np.any(neck_mask):
                # Find the bounding box of the neck mask
                neck_coords = np.where(neck_mask)
                if len(neck_coords[0]) > 0:
                    min_y, max_y = np.min(neck_coords[0]), np.max(neck_coords[0])
                    neck_height = max_y - min_y
                    
                    # Calculate the cutoff point based on neck_mask_radius
                    # neck_mask_radius=0.3 means mask bottom 30% of neck, leave top 70% unmasked
                    cutoff_y = min_y + int(neck_height * (1 - neck_mask_radius))
                    
                    # Create a mask for the specified portion of the neck
                    neck_mask_modified = neck_mask.copy()
                    neck_mask_modified[:cutoff_y, :] = 0  # Remove top portion
                    neck_mask = neck_mask_modified
...
def center_crop(image):
    """Center crop image to 3:4 aspect ratio"""
